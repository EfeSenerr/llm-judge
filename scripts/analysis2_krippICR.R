...
df <- df %>%
    filter(source %in% c("heval3", "heval4")) %>% # select eligible
    select(evaluator, matches("^(atc_|smm_)"), datapoint_new)
# select(evaluator, all_of(entity_name), datapoint_new)
...
write.csv(alpha_table, "output/alpha_results_krippICR.csv", row.names = FALSE)

write.xlsx(alpha_table,
    file = "output/output/alpha_results_krippICR.xlsx",
    asTable = FALSE,
    overwrite = TRUE,
    colNames = TRUE
)
