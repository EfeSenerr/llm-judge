...
ggplot(df_analysis, aes(x = Summary_SPD_human, fill = Summary_Consistency_human)) +
  ...
  labs(title = "Distribution of SPD Sentiment Ratings by Consistency (Human)",
...
ggplot(tab_df, aes(x = Sentiment, y = Consistency, fill = Count)) +
  ...
  labs(title = "Heatmap: SPD Sentiment vs. Consistency (Human)",
...
mosaicplot(tab, main = "Mosaic Plot: SPD Sentiment vs. Consistency (Human)",
...
df_analysis <- df_analysis %>%
  mutate(across(matches("^Summary_(SPD|FDP|die_Gruenen|Olaf_Scholz|Christian_Lindner|Robert_Habeck)_(human|llm)$"),
...
ggplot(gamma_df, aes(x = Gamma, y = Subject)) +
  geom_point(size = 3) +
  labs(title = "Goodman and Kruskal's Gamma by Subject",
